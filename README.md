...
| 场景 | 没有本工具时 | 有了 Profile Isolator |
|------|--------------|------------------------|
| 终端 A 用供应商 X 的 Codex | 改全局 `~/.codex` | 选 profile「供应商 X」→ Launch |
| 终端 B 同时用供应商 Y 的 Codex | 再改全局，A 也乱了 | 另开 profile「供应商 Y」→ Launch |
| Claude Code 也要换供应商 | 再改 `~/.claude` | 切到 Claude 页，同样按 profile 启动 |
| 同一项目在不同供应商间 resume | 会话散落/对不上 | 会话可共享到同一 `~/.codex`，按工作目录 resume |

**一句话：多个 Codex / Claude Code 进程并行，各自走不同供应商，配置隔离、会话可选共享。**
